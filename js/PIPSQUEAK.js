...
$(document).ready(function () {
    ...
    $('#durationSubmit').click(function () {
        ...
        if (dur > (Number(document.getElementById("totalTime").innerHTML) - Number(document.getElementById("currentTime").innerHTML))) {
    ...
    //************************************THIS IS WHERE WE SEND THE INFO TO THE BACK END************************************
            //var sendToBackend = [ startLine, " ", endLine, " ", startTime, " ", endTime, " ",action]; //print to test outpu
    ...
    $('#autoScrollSubmit').click(function () {
        ...
        var autoScrollNum = parseInt($('#autoScrollLine').val(), 10),
            error = "";
    ...
    $('#annotateSubmit').click(function () {
        annotateLineNum = parseInt($('#annotateLine').val(), 10);
        var error = "";
        ...
        if (profComments[annotateLineNum] === undefined) { //check if there is already a comment on the line
    ...
    pub.printDir = function (directory, level, out) {
        ...
        for (i = 0; i < files.length; i += 1) {
            ...
            if (fs.lstatSync(directory + "/" + files[i]).isDirectory()) {
                out.push(files[i] + "\n");
                out = this.printDir((directory + "/" + files[i]), level + 1, out);
            } else {
                out.push(files[i] + "\n");
            }
        }
...
var squeak = (function () {
    ...
    pub.publish = function (media, fileContents, name, path) {
        ...
        if (fs.existsSync(path + "/" + name + "/index.html")) {
        ...
        firstPart = pip.readFile("./templates/firstPart.txt");
        ...
        secondPart = pip.readFile("./templates/secondPart.txt");
        html += secondPart;
        for (i = 0; i < fileContents.length; i += 1) {
            html += fileContents[i];
        }
        thirdPart = pip.readFile("./templates/thirdPart.txt");
        html += thirdPart;
        html += name;
        fourthPart = pip.readFile("./templates/fourthPart.txt");
        html += fourthPart;
       ...
       // pip.copyDir("./node_modules", path + "/" + name + "/node_modules/");
        ...
        setTimeout(function () {
            newWin = gui.Window.open(path + "/" + name + "/index.html", {
                position: 'center',
                width: 1280,
                height: 720,
                'new-instance': true
